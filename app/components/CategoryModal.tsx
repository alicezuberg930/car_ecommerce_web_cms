...
const CategoryModal: React.FC<{ selectedCategory?: Category, setSelected?: Dispatch<SetStateAction<Category | null>> }> = ({ selectedCategory = null, setSelected }) => {
    const [images, setImages] = useState<{ file: File | null, url: string }[]>([])
    const { data: categories, isLoading } = readCategoryHook()
    const createHook = createCategoryHook()
    const updateHook = updateCategoryHook()
    ...
    const handleCategoryAction = async (e: FormEvent<HTMLFormElement>) => {
        ...
        if (selectedCategory != null) {
            ...
            if (isSubcategory) getSubCategories(currentParentCategory!).then(res => { dispatch(setSubCategories(res.data)) })
        } else {
                                        ...
                                        <select defaultValue={selectedCategory?.parentCategory ?? ""} className='border-gray-300 p-2 border rounded-md w-full outline-none' name="parentCategory">
                                            <option value="" className="text-lg">Không chọn</option>
                                            {
                                                isLoading ?
                                                    ...
                                                    categories && <CategorySelectList categories={categories?.data.categories as Category[]} currentPage={1} />
                                            }
                                        </select>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <div className='space-x-3 font-bold text-md'>
                                            ...
                                            {selectedCategory && <button type="button" onClick={() => setSelected!(null)} className='bg-[#eeeeee] p-3 rounded-md outline-none'>Đóng</button>}
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
